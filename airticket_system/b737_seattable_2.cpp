...
B737_seatTable_2::B737_seatTable_2(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::B737_seatTable_2)
{
    ui->setupUi(this);
...
B737_seatTable_2::~B737_seatTable_2()
{
    delete ui;
}

void B737_seatTable_2::createArray()
{
    X = new int[amount];
    Y = new int[amount];
...
void B737_seatTable_2::exchange(QString seatnum)
{
    QString i,j;
    if(seatnum.length()>2)
    {
...
void B737_seatTable_2::fillTable()
{
    for(int i =0;i<25;i++)
    {
        seat_table[i][3] = 3;
...
void B737_seatTable_2::paintEvent(QPaintEvent *)
{
...
void B737_seatTable_2::mouseMoveEvent(QMouseEvent * event)
{
    moveX=event->x();
    moveY=event->y();
    int i,j;
...
void B737_seatTable_2::mousePressEvent(QMouseEvent *)
{
    if(seat_table[a][b] == 0)
    {
        seat_table[a][b] = 2;
...
QString B737_seatTable_2::numToChar(int num)
{
    QString c;
    if(num == 0)
        c = "A";
...
void B737_seatTable_2::on_pushButton_2_clicked()  //取消按钮
{
    this->close();
}

void B737_seatTable_2::on_pushButton_clicked()  //确认按钮
{
    if(count != amount)
    {
